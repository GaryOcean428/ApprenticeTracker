      ...
      return (
        <Card className="w-full max-w-3xl mx-auto my-8 border-destructive">
          ...
          <CardContent className="pt-6">
            ...
            <Button onClick={() => (window.location.href = redirectTo)}>Return to Safe Page</Button>
          </CardContent>
        </Card>
      );
    }
...
export const idParamSchema = z.object({
  id: z.string().refine(val => /^\d+$/.test(val) && parseInt(val) > 0, {
...
export const optionalIdParamSchema = z.object({
  id: z
    .string()
    .optional()
    .refine(val => val === undefined || (/^\d+$/.test(val) && parseInt(val) > 0), {
