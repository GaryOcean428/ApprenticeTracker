...
export default function FairWorkUpdateNotification({ 
  ...
  const { data: awardUpdates, isLoading, error } = useQuery({
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-amber-800 dark:text-amber-300">Fair Work Award Updates Available</CardTitle>
          </div>
          <Badge variant="outline" className="bg-amber-200 text-amber-800 dark:bg-amber-800 dark:text-amber-200">
            {newAwards.length + updatedAwards.length} update{(newAwards.length + updatedAwards.length) > 1 ? 's' : ''}
          </Badge>
        </div>
