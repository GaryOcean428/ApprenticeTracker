...
export default function EnhancedAwardSelector({
  ...
  useEffect(() => {
    if (awardsData?.success && selectedAwardCode) {
      const selectedAward = awardsData.data.find((award: Award) => award.code === selectedAwardCode);
                    ...
                    <div className="flex flex-col sm:flex-row justify-between">
                      <div>
                        <h3 className="text-lg font-medium">
                          {rateData.data.award.name}
                        </h3>
                        <p className="text-sm text-muted-foreground">
                          {getOrdinalSuffix(rateData.data.parameters.apprenticeYear)} Year Apprentice ({selectedYear})
                        </p>
                      </div>
                      <div className="mt-2 sm:mt-0">
                        <Badge variant="outline" className="bg-primary/10 text-primary">
                          {selectedSector === 'standard' ? 'Standard' : selectedSector.charAt(0).toUpperCase() + selectedSector.slice(1)} Sector
                        </Badge>
                      </div>
                    </div>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              <TableRow>
                                ...
                                <TableCell className="text-right">{formatCurrency(rateData.data.calculationFactors.baseRate)}</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell>Adult Loading</TableCell>
                                <TableCell className="text-right">{formatCurrency(rateData.data.calculationFactors.adultBonus)}</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell>Year 12 Completion</TableCell>
                                <TableCell className="text-right">{formatCurrency(rateData.data.calculationFactors.year12Bonus)}</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell>Sector Multiplier</TableCell>
                                <TableCell className="text-right">{rateData.data.calculationFactors.sectorMultiplier}x</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell>Year Adjustment</TableCell>
                                <TableCell className="text-right">{(rateData.data.calculationFactors.yearAdjustment * 100).toFixed(1)}%</TableCell>
                              </TableRow>
                              <TableRow className="font-medium">
                                <TableCell>Final Hourly Rate</TableCell>
                                <TableCell className="text-right">{formatCurrency(rateData.data.rates.hourly)}</TableCell>
                              </TableRow>
                            </TableBody>
                          </Table>
                        </div>
                            ...
                            <TableBody>
                              {historicalData.data.historicalRates.map((record) => (
                                <TableRow key={record.year}>
                                  ...
                                  <TableCell className="text-right">{formatCurrency(record.rates.hourly)}</TableCell>
                                  <TableCell className="text-right">{formatCurrency(record.rates.weekly)}</TableCell>
                                  <TableCell className={`text-right ${getPercentageChangeColor(record.percentageChange)}`}>
                                    {record.percentageChange !== null ? `${record.percentageChange}%` : '-'}
                                  </TableCell>
                                </TableRow>
                              ))}
                            </TableBody>
                          ...
                          <div className="space-y-6">
                            ...
                            <div>
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {allowancesData.data.allowances.industry.map((allowance, index) => (
                                    <TableRow key={index}>
                                      <TableCell>
                                        {allowance.name}
                                        {allowance.trade && <span className="text-xs text-muted-foreground ml-1">({allowance.trade})</span>}
                                        {allowance.description && (
                                          <TooltipProvider>
                                            <Tooltip>
                                              ...
                                              <TooltipContent>
                                                <p className="max-w-xs">{allowance.description}</p>
                                              </TooltipContent>
                                            </Tooltip>
                                          </TooltipProvider>
                                        )}
                                      ...
                                      <TableCell className="text-right">{allowance.period}</TableCell>
                                    </TableRow>
                                  ))}
                                </TableBody>
                              </Table>
                            ...
                            <div>
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {allowancesData.data.allowances.expense.map((allowance, index) => (
                                    <TableRow key={index}>
                                      <TableCell>
                                        {allowance.name}
                                        {allowance.description && (
                            ...
                            {allowancesData.data.allowances.skill && (
                              <div>
                                ...
                                <Table>
                                  ...
                                  <TableBody>
                                    {allowancesData.data.allowances.skill.map((allowance, index) => (
                                      <TableRow key={index}>
                                        <TableCell>
                                          {allowance.name}
                                          {allowance.description && (
                          ...
                          <div className="space-y-6">
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  <TableRow>
                                    ...
                                    <TableCell className="text-right">{penaltyRatesData.data.penaltyRates.overtime.firstTwoHours}x</TableCell>
                                    <TableCell className="text-right">
                                      {rateData?.success && 
                                        formatCurrency(rateData.data.rates.hourly * penaltyRatesData.data.penaltyRates.overtime.firstTwoHours)}
                                    </TableCell>
                                  </TableRow>
                                  <TableRow>
                                    <TableCell>After two hours</TableCell>
                                    <TableCell className="text-right">{penaltyRatesData.data.penaltyRates.overtime.afterTwoHours}x</TableCell>
                                    <TableCell className="text-right">
                                      {rateData?.success && 
                                        formatCurrency(rateData.data.rates.hourly * penaltyRatesData.data.penaltyRates.overtime.afterTwoHours)}
                                    </TableCell>
                                  </TableRow>
                                  <TableRow>
                                    <TableCell>Sunday</TableCell>
                                    <TableCell className="text-right">{penaltyRatesData.data.penaltyRates.overtime.sunday}x</TableCell>
                                    <TableCell className="text-right">
                                      {rateData?.success && 
                                        formatCurrency(rateData.data.rates.hourly * penaltyRatesData.data.penaltyRates.overtime.sunday)}
                                    </TableCell>
                                  </TableRow>
                                  <TableRow>
                                    <TableCell>Public Holiday</TableCell>
                                    <TableCell className="text-right">{penaltyRatesData.data.penaltyRates.overtime.publicHoliday}x</TableCell>
                                    <TableCell className="text-right">
                                      {rateData?.success && 
                                        formatCurrency(rateData.data.rates.hourly * penaltyRatesData.data.penaltyRates.overtime.publicHoliday)}
                                    </TableCell>
                                  </TableRow>
                                </TableBody>
                              </Table>
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  <TableRow>
                                    ...
                                    <TableCell className="text-right">{penaltyRatesData.data.penaltyRates.weekend.saturday}x</TableCell>
                                    <TableCell className="text-right">
                                      {rateData?.success && 
                                        formatCurrency(rateData.data.rates.hourly * penaltyRatesData.data.penaltyRates.weekend.saturday)}
                                    </TableCell>
                                  </TableRow>
                                  <TableRow>
                                    <TableCell>Sunday</TableCell>
                                    <TableCell className="text-right">{penaltyRatesData.data.penaltyRates.weekend.sunday}x</TableCell>
                                    <TableCell className="text-right">
                                      {rateData?.success && 
                                        formatCurrency(rateData.data.rates.hourly * penaltyRatesData.data.penaltyRates.weekend.sunday)}
                                    </TableCell>
                                  </TableRow>
                                </TableBody>
                              </Table>
                            ...
                            <div>
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {Object.entries(penaltyRatesData.data.penaltyRates.shiftWork).map(([key, value]) => (
                                    <TableRow key={key}>
                                      <TableCell>
