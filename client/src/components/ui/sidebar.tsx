...
const SidebarItem = ({ icon, label, href, children, active }: SidebarItemProps) => {
  ...
  return (
    <li className="relative px-6 py-2">
      {isActive && (
        <span
          className="absolute inset-y-0 left-0 w-1 bg-[#0070F3] rounded-tr-lg rounded-br-lg"
          aria-hidden="true"
        ></span>
      )}
            ...
            <ul className="pl-2 pt-1 pb-1 mt-1 space-y-1 overflow-hidden text-xs rounded-md">
              {children?.map((child, index) => {
                ...
                return (
                  <li
                    key={index}
                    className="px-2 py-1 transition-colors duration-150 hover:text-primary"
                  >
                    <Link
                      href={child.href}
                      className={cn(
      ...
      <div className="py-4 max-h-screen overflow-y-auto">
        ...
        <ul className="mt-6">
          ...
          <SidebarItem
            ...
            active={
              location.startsWith('/timesheets') ||
              location.startsWith('/awards') ||
              location === '/fair-work-demo'
            }
            children={[
