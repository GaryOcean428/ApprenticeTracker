...
import { PlusCircle, Search, Filter, Calendar, User, Check, X, Upload, SlidersHorizontal, Info } from "lucide-react";
...
export default function CompetencyReview() {
  ...
  const uniqueApprentices = [
    ...new Set((reviews || []).map((review) => ({ id: review.apprenticeId, name: review.apprenticeName }))),
  ];
  const uniqueHosts = [
    ...new Set((reviews || []).map((review) => ({ id: review.hostId, name: review.hostName }))),
  ];
  const uniqueDates = [
    ...new Set((reviews || []).map((review) => review.date)),
  ].sort((a, b) => new Date(b).getTime() - new Date(a).getTime());

  // Filter reviews based on selected filters
  const filteredReview = reviews?.find((review) => {
    const matchesApprentice = selectedApprentice ? review.apprenticeId.toString() === selectedApprentice : false;
    ...
    <>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <Select
          ...
          onValueChange={setSelectedApprentice}
        >
          <SelectTrigger>
            <span className="flex items-center">
              <User className="mr-2 h-4 w-4" />
              <span>{selectedApprentice ? uniqueApprentices.find(a => a.id.toString() === selectedApprentice)?.name : "Select Apprentice"}</span>
            </span>
          </SelectTrigger>
          <SelectContent>
        ...
        >
          <SelectTrigger>
            <span className="flex items-center">
              ...
              <span>{selectedHost ? uniqueHosts.find(h => h.id.toString() === selectedHost)?.name : "Select Host (Optional)"}</span>
            </span>
          </SelectTrigger>
          <SelectContent>
      ...
      {!selectedApprentice ? (
        ...
        <Card>
          <CardHeader className="p-4 border-b">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <div>
                ...
                <CardDescription>
                  Host Employer: {filteredReview.hostName} | Date: {format(new Date(filteredReview.date), "dd MMM yyyy")} | Field Officer: {filteredReview.fieldOfficer}
                </CardDescription>
              </div>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  ...
                                  size="icon"
                                  className={unit.competent ? "bg-green-500 hover:bg-green-600" : ""}
                                  onClick={() => toggleCompetency(filteredReview.id, unit.id, unit.competent)}
                                >
                                  {unit.competent ? (
                        ...
                        <TableCell className="text-center">
                          {unit.hasEvidence ? (
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => handleUploadEvidence(filteredReview.id, unit.id, unit.code, unit.title)}
          ...
          <CardFooter className="flex justify-end p-4 border-t gap-2">
            <Button variant="outline" onClick={() => navigate(`/field-officers/competency/${filteredReview.id}/edit`)}>
              Edit Review
            </Button>
            <Button onClick={() => navigate(`/field-officers/competency/${filteredReview.id}/report`)}>
              Generate Report
            </Button>
          </CardFooter>
        </Card>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={simulateUpload} disabled={isUploading[`${selectedUnit?.reviewId}-${selectedUnit?.unitId}`]}>
