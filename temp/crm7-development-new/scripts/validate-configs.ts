...
function validateESLintConfig(): { errors: string[]; warnings: string[] } {
  const errors: string[] = [];
  ...
  try {
    ...
    Object.entries(criticalRules).forEach(([rule, expectedLevel]) => {
      ...
      } else if (typeof currentLevel === 'string' && currentLevel !== expectedLevel) {
        warnings.push(
          `Rule ${rule} has level "${String(currentLevel)}" but "${expectedLevel}" is recommended`
        );
      }
    });
...
async function main(): Promise<void> {
  try {
    const { errors, warnings } = validateESLintConfig();
