...
export async function importContactsFromEmployeeCSV() {
  try {
    ...
    // Read CSV file
    const csvFilePath = join(process.cwd(), 'attached_assets', 'EmployeeDetail - EmployeeDetail.csv');
    ...
    const contactsToImport = records.slice(0, 50).map(record => ({
      ...
      email: record.EMP_EMAIL || `${record.EMP_GIVENNAMES?.toLowerCase() || 'unknown'}${record.EMP_SURNAME?.toLowerCase() || 'user'}@example.com`,
      ...
      country: "Australia",
      contactType: "person",
      primaryRole: record.EMP_APPRENTICESHIP === 'T' ? "apprentice" : 
                  record.EMP_TRAINEE === 'T' ? "trainee" : "worker",
...
export async function importClientContactsFromCSV() {
  try {
    ...
    // Read CSV file
    const csvFilePath = join(process.cwd(), 'attached_assets', 'Search All Clients - Search All Clients.csv');
    const fileContent = readFileSync(csvFilePath, { encoding: 'utf-8' });
  ...
  try {
    ...
    allContacts.forEach(contact => {
      if (contact.primaryRole === "apprentice" || contact.primaryRole === "trainee") {
        ...
        // Don't add to any group yet
      } else if (contact.companyName?.toLowerCase().includes("rto") && groupIdsByName["RTO Coordinators"]) {
