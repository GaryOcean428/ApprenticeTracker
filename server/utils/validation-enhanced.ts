...
export const hourlyRateSchema = z.number().min(0.01, "Hourly rate must be at least $0.01").max(1000, "Hourly rate cannot exceed $1000");
...
export const pastOrPresentDateSchema = z.date().refine(
  (date) => date <= new Date(),
...
export const createApprenticeValidationSchema = z.object({
...
export const createTimesheetDetailValidationSchema = z.object({
  date: pastOrPresentDateSchema,
  hoursWorked: z.number().min(0.1, "Hours worked must be at least 0.1").max(12, "Hours worked cannot exceed 12 per day"),
...
export const createPayRateValidationSchema = z.object({
...
export const createHostEmployerValidationSchema = z.object({
  ...
  employerIdentifier: z.string().optional().refine(
    (abn) => !abn || validateABN(abn),
    "Invalid ABN format"
  )
});
...
export class BusinessRuleValidator {
  static validateTimesheetApproval(timesheet: any, user: any): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  static validatePlacementCreation(apprentice: any, hostEmployer: any): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  static validateProgressReviewScheduling(apprentice: any, reviewer: any, reviewDate: Date): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
