...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/contacts/tags/all', directEndpointAuth, async (req, res) => {
    try {
      ...
      console.log(
        `Returning ${formattedTags.length} tags from direct /api/contacts/tags/all endpoint`
      );
  ...
  app.post(
    '/api/contacts/tags',
    directEndpointAuth,
    directEndpointPermission('contacts.manage'),
    async (req, res) => {
      try {
  ...
  app.post(
    '/api/contacts/groups/:id/members',
    directEndpointAuth,
    directEndpointPermission('contacts.manage'),
    async (req, res) => {
      try {
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        console.log(
          'DELETE /api/contacts/groups/:groupId/members/:contactId direct endpoint called'
        );
        ...
        console.error(
          `Error in direct DELETE /api/contacts/groups/:groupId/members/:contactId:`,
          error
        );
        res.status(500).json({ message: error.message });
      }
  ...
  app.get('/api/contacts', directEndpointAuth, async (req, res) => {
    try {
      ...
      console.log(
        `Returning ${paginatedContacts.length} contacts (page ${page}, limit ${limit}, total ${total})`
      );
  ...
  app.post('/api/users/:id/subscription', async (req, res) => {
    try {
      ...
      const { subscriptionPlanId, stripeCustomerId, stripeSubscriptionId, startDate, endDate } =
        req.body;
      ...
      const isActive =
        roleName === 'developer' ? true : !!endDate && new Date(endDate) > new Date();
  ...
  app.get('/api/dashboard/metrics', async (req, res) => {
    try {
      ...
      try {
        ...
        metrics.complianceAlerts = complianceRecords.filter(
