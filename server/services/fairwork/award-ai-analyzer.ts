...
export class AwardAIAnalyzer {
      ...
      try {
        ...
        aiResult = await this.openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert in Australian employment law and Fair Work Commission awards. You need to analyze changes between award versions and determine their impact for employers."
            },
            {
              role: "user",
        ...
        aiResult = await this.openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert in Australian employment law and Fair Work Commission awards."
            },
            {
              role: "user",
              content: `Analyze the changes between version ${oldVersion} and ${newVersion} of the Fair Work award ${awardCode} (${awardName}).
    ...
    try {
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an assistant that generates clear, concise notifications about Fair Work award updates for Australian businesses."
          },
          {
            role: "user",
  ...
  private generateFallbackNotification(
    ...
    const impactText = impactLevel === 'high' 
      ? 'important' 
      : impactLevel === 'medium' 
        ? 'significant' 
        : 'minor';
