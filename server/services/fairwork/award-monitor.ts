...
export class AwardMonitorService {
   ...
   */
  private async sendAwardUpdateNotification(
    newAwards: any[],
    updatedAwards: any[]
  ): Promise<void> {
    try {
      // Create notification content
      let emailSubject = 'Fair Work Award Updates Available';
      ...
      emailContent += '<p>To update these awards in the system, please access the Award Management section in the admin dashboard.</p>';
  ...
  public async getAwardUpdates(
    ...
    try {
      let query = db
        .select()
        .from(awardUpdateChecks)
        .orderBy(desc(awardUpdateChecks.checkDate));
