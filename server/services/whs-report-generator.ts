...
export async function generateIncidentPDFReport(reportData: any): Promise<Buffer> {
  try {
    ...
    doc.setFontSize(12);
    doc.text(
      `Generated: ${new Date(reportData.summary.reportGeneratedAt).toLocaleDateString()}`,
      20,
      45
    );
...
export async function generateIncidentExcelReport(reportData: any): Promise<Buffer> {
  try {
    ...
    summarySheet.addRow([
      'Generated:',
      new Date(reportData.summary.reportGeneratedAt).toLocaleDateString(),
    ]);
    summarySheet.addRow([]);
  ...
  try {
    ...
    const tableData = assessments.map((assessment: any) => [
      ...
      assessment.approval_date
        ? new Date(assessment.approval_date).toLocaleDateString()
        : 'Pending',
    ]);

    doc.autoTable({
      head: [
        ['Title', 'Status', 'Assessment Date', 'Location', 'Assessor', 'Host Employer', 'Approved'],
      ],
      body: tableData,
      startY: 70,
      styles: { fontSize: 8, cellPadding: 2 },
