...
export class AwardRateCalculator {
  ...
  async getApprenticePayRate(
    ...
    try {
      logger.info(
        `Getting apprentice pay rate for: Award ${awardCode}, Year ${apprenticeYear}, Adult: ${isAdult}, Year 12: ${hasCompletedYear12}`
      );
      ...
      if (apprenticeRates && apprenticeRates.length > 0) {
        ...
        logger.info(
          `Found apprentice pay rate: $${rate.hourly_rate}/hr (from ${rate.effective_from})`
        );
      ...
      try {
        logger.info(
          `No local apprentice pay rate found, trying Fair Work API for award ${awardCode}`
        );
        ...
        const matchingRate = fairWorkRates.find(
          rate => rate.is_apprentice_rate && rate.apprenticeship_year === apprenticeYear
        );
        ...
        const referenceClassification = await this.getReferenceTradeClassification(
          award.id,
          awardCode
        );
        ...
        if (referenceClassification) {
          ...
          if (referenceRates && referenceRates.length > 0) {
            ...
            if (isAdult) {
              ...
              switch (apprenticeYear) {
                ...
                case 4:
                  ...
                  break; // 95%
                default:
                  percentage = 0.5; // Default fallback
              }
            } else {
            ...
            logger.info(
              `Calculated apprentice pay rate: $${calculatedRate}/hr (${percentage * 100}% of $${referenceHourlyRate}/hr)`
            );
        ...
        const calculatedRate = this.calculateDefaultApprenticeRate(
          apprenticeYear,
          isAdult,
          hasCompletedYear12
        );
        ...
        const fallbackRate = this.calculateDefaultApprenticeRate(
          apprenticeYear,
          isAdult,
          hasCompletedYear12
        );
   ...
   */
  async getClassificationPayRate(
    awardCode: string,
    classificationCode: string
  ): Promise<number | null> {
    try {
      logger.info(
        `Getting classification pay rate for: Award ${awardCode}, Classification ${classificationCode}`
      );
      ...
      if (classificationRates && classificationRates.length > 0) {
        ...
        logger.info(
          `Found classification pay rate: $${rate.hourly_rate}/hr (from ${rate.effective_from})`
        );
        return parseFloat(String(rate.hourly_rate));
      }
      ...
      try {
        logger.info(
          `No local classification pay rate found, trying Fair Work API for award ${awardCode}`
        );
        ...
        const validRates = fairWorkRates.filter(
          rate =>
            !rate.is_apprentice_rate && (!rate.effective_to || rate.effective_to >= currentDate)
        );
        ...
        if (validRates.length > 0) {
          ...
          // Cache this rate in the database for future use
          await this.cacheClassificationPayRate(award.id, classification.id, apiRate);
        ...
        // Cache this rate in the database for future use
        await this.cacheClassificationPayRate(award.id, classification.id, calculatedRate);
  ...
  async getEnterpriseAgreementRate(
    ...
    try {
      logger.info(
        `Getting enterprise agreement rate for: Agreement ${agreementId}, Classification ${classificationCode}`
      );
    ...
    try {
      ...
      for (const classification of apprenticeClassifications) {
        ...
        if (
          ...
          fairWorkCode.includes('ap') ||
          fairWorkCode.includes('tr')
        ) {
      ...
      if (awardCode === 'MA000025') {
        ...
        return classifications.find(
          c =>
            (c.name || '')
              .toLowerCase()
              .includes('plumbing and mechanical services tradesperson') ||
  ...
  private calculateDefaultApprenticeRate(
    ...
    if (isAdult) {
      ...
      switch (apprenticeYear) {
        case 1:
          return 23.91; // 1st year adult apprentice
        case 2:
          return 26.42; // 2nd year adult apprentice
        case 3:
          return 26.42; // 3rd year adult apprentice
        case 4:
          return 26.42; // 4th year adult apprentice
        default:
          return 23.91; // Default to 1st year
      }
    } else {
      ...
      if (hasCompletedYear12) {
        ...
        switch (apprenticeYear) {
          case 1:
            return 16.62; // 1st year with Year 12
          case 2:
            return 19.53; // 2nd year with Year 12
          case 3:
            return 20.99; // 3rd year with Year 12
          case 4:
            return 24.49; // 4th year with Year 12
          default:
            return 16.62; // Default to 1st year
        }
      } else {
        // Without Year 12 completion
        switch (apprenticeYear) {
          case 1:
            return 15.16; // 1st year without Year 12
          case 2:
            return 18.08; // 2nd year without Year 12
          case 3:
            return 20.99; // 3rd year without Year 12
          case 4:
            return 24.49; // 4th year without Year 12
          default:
            return 15.16; // Default to 1st year
        }
      }
    }
  }
  ...
  private async cacheApprenticePayRate(
    ...
    try {
      ...
      logger.info(
        `Cached apprentice pay rate: $${hourlyRate}/hr for award ${awardId}, year ${apprenticeYear}`
      );
    } catch (error) {
  ...
  private async cacheClassificationPayRate(
    ...
    try {
      ...
      await db.execute(query);
      logger.info(
        `Cached classification pay rate: $${hourlyRate}/hr for award ${awardId}, classification ${classificationId}`
      );
    } catch (error) {
