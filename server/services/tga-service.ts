...
export class TGAService {
  ...
  ): Promise<TGAQualificationDetail> {
    try {
      console.log(
        `Getting qualification details for code: "${code}", includeUnits: ${includeUnits}`
      );
      ...
      throw new Error(
        `Failed to fetch TGA qualification: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
    ...
    try {
      ...
      if (results.length === 0 && (!soapClientError || soapClientError)) {
        try {
          ...
          throw new Error(
            `Failed to retrieve qualification data from Training.gov.au: ${errorMessage}`
          );
        }
      }
      ...
      throw new Error(
        `Failed to search TGA qualifications: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
                ...
                : code.startsWith('SIT')
                  ...
                  : code.startsWith('ICT')
                    ...
                    : code.startsWith('AHC')
                      ...
                      : `Qualification ${code}`,
          description:
            'This qualification reflects the role of individuals in a variety of roles who use well-developed skills and a broad knowledge base in a wide variety of contexts.',
            ...
            core: [
              {
                ...
                status: 'Current',
                description:
                  'Apply safety protocols in the workplace environment following industry standards and procedures',
  ...
  private async getQualificationBySoap(code: string): Promise<TGAQualificationDetail | null> {
    try {
      ...
      const coreUnits: TGAUnitOfCompetency[] = [];
      const electiveUnits: TGAUnitOfCompetency[] = [];
      ...
      if (
        ...
        for (const rel of relationships) {
          if (rel.RelatedComponentCode && rel.RelatedComponentTitle) {
            ...
            const isCore =
              rel.Attributes &&
              rel.Attributes.some((attr: any) => attr.Name === 'UnitFlag' && attr.Value === 'C');
  ...
  async getUnitOfCompetencyByCode(code: string): Promise<TGAUnitOfCompetency | null> {
    try {
      ...
      try {
        ...
        throw new Error(
          `Unable to retrieve unit of competency data from Training.gov.au API: ${errorMessage}. Please ensure valid API credentials are provided.`
        );
      }
    } catch (error: unknown) {
      console.error(`Error fetching TGA unit ${code}:`, error);
      throw new Error(
        `Failed to fetch TGA unit of competency: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
      ...
      if (!existingQualification) {
        ...
        const newQualificationData = {
          ...
          qualificationDescription:
            ...
            `Imported from Training.gov.au on ${new Date().toLocaleDateString()}`,
  ...
  private async importUnit(
    ...
    try {
      ...
      if (existingUnits.length === 0) {
        ...
        const newUnits = await db.insert(unitsOfCompetency).values(newUnitData).returning();
      ...
      if (existingStructures.length === 0) {
        ...
        console.log(
          `Linked unit ${unitData.code} to qualification as ${isCore ? 'core' : 'elective'}`
        );
      } else {
  ...
  async syncQualifications(searchQuery: string, limit: number = 20): Promise<number> {
    try {
      ...
      try {
        ...
        console.log(
          `Found ${qualificationResults.length} qualifications matching "${searchQuery}"`
        );
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        ...
        throw new Error(
          `Failed to retrieve qualification data from Training.gov.au: ${errorMessage}. Please ensure valid API credentials are provided.`
        );
      }
      ...
      console.log(
        `Successfully imported ${importedCount} of ${qualificationResults.length} qualifications`
      );
