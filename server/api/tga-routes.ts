...
import {
  ...
  validateBody,
  tgaSearchSchema,
  tgaQualificationSchema,
  tgaQualificationImportSchema,
  tgaSyncSchema,
  idParamSchema,
  tgaSyncBatchSchema,
  tgaSyncAllSchema,
...
export function registerTGARoutes(app: Express) {
      ...
      try {
        ...
        try {
          ...
          const searchResults = allQualifications.filter(qualification => {
            ...
            return (
              title.includes(searchTerm) || code.includes(searchTerm) || desc.includes(searchTerm)
            );
          });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          console.warn(
            `TGA API search failed, falling back to sample data: ${apiError instanceof Error ? apiError.message : 'Unknown error'}`
          );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        console.warn(
          `Failed to fetch qualification from TGA API: ${apiError instanceof Error ? apiError.message : 'Unknown error'}`
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (existingQual.length > 0 && skipExisting) {
          ...
          console.log(
            `Qualification ${code} already exists with ID ${qualificationId} - skipping import`
          );
        }
        ...
        // If it doesn't exist, create it
          ...
          try {
            ...
            try {
              ...
              console.warn(
                `Failed to get qualification data from TGA API: ${apiError instanceof Error ? apiError.message : 'Unknown error'}`
              );
            ...
            const [result] = await db
              ...
              .values({
                ...
                qualificationDescription:
                  qualificationData.description || 'Imported from Training.gov.au',
                trainingPackageCode:
                  qualificationData.trainingPackage?.code || code.substring(0, 3),
            ...
            if (importUnits && qualificationData.units && qualificationData.units.length > 0) {
              console.log(
                `Importing ${qualificationData.units.length} units for qualification ${code}`
              );
   ...
   */
  app.post('/api/tga/sync', validateBody(tgaSyncSchema), async (req: Request, res: Response) => {
    try {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const coreUnits = structure.filter(item => item.isCore === true).map(item => item.unit);
        ...
        console.log(
          `Found ${coreUnits.length} core units and ${electiveUnits.length} elective units`
        );
