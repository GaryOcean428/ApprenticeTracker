...
router.get('/templates', async (req: Request, res: Response) => {
  try {
    const templates = await db.select().from(progressReviewTemplates).orderBy(desc(progressReviewTemplates.updatedAt));
...
router.put('/templates/:id', canManageProgressReviews, async (req: Request, res: Response) => {
  try {
    ...
    const [updatedTemplate] = await db.update(progressReviewTemplates)
      .set(validatedData)
      .where(eq(progressReviewTemplates.id, id))
      .returning();
...
router.delete('/templates/:id', canManageProgressReviews, async (req: Request, res: Response) => {
  try {
    ...
    await db.update(progressReviewTemplates)
      .set({ isActive: false })
      .where(eq(progressReviewTemplates.id, id));
...
router.get('/reviews/:id', async (req: Request, res: Response) => {
  try {
    ...
    // Fetch participants
    const participants = await db.select().from(progressReviewParticipants)
      .where(eq(progressReviewParticipants.reviewId, id));
      
    // Fetch action items
    const actionItems = await db.select().from(progressReviewActionItems)
      .where(eq(progressReviewActionItems.reviewId, id));
      
    // Fetch associated documents
    const documentLinks = await db.select().from(progressReviewDocuments)
      .where(eq(progressReviewDocuments.reviewId, id));
      
    const documentIds = documentLinks.map(link => link.documentId);
    const reviewDocuments = documentIds.length > 0 ? 
...
router.put('/reviews/:id', async (req: Request, res: Response) => {
  try {
    ...
    const validatedData = insertProgressReviewSchema.omit({
      createdAt: true,
      updatedAt: true
    }).parse({
      ...req.body,
    ...
    const [updatedReview] = await db.update(progressReviews)
      .set(validatedData)
      .where(eq(progressReviews.id, id))
      .returning();
...
router.delete('/reviews/:id', canManageProgressReviews, async (req: Request, res: Response) => {
  try {
    ...
    await db.update(progressReviews)
...
router.post('/reviews/:reviewId/participants', async (req: Request, res: Response) => {
  try {
    ...
    const [participant] = await db.insert(progressReviewParticipants).values(validatedData).returning();
...
router.put('/participants/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [updatedParticipant] = await db.update(progressReviewParticipants)
...
// Remove participant
router.delete('/participants/:id', canManageProgressReviews, async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
...
router.post('/reviews/:reviewId/action-items', async (req: Request, res: Response) => {
  try {
    ...
    const [actionItem] = await db.insert(progressReviewActionItems).values(validatedData).returning();
...
router.put('/action-items/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [updatedActionItem] = await db.update(progressReviewActionItems)
      .set(updateData)
      .where(eq(progressReviewActionItems.id, id))
      .returning();
...
// Delete action item
router.delete('/action-items/:id', canManageProgressReviews, async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
...
router.get('/dashboard/upcoming', async (req: Request, res: Response) => {
  try {
    ...
    const reviewerReviews = await db.select().from(progressReviews)
      .where(
        and(
          eq(progressReviews.reviewerId, user.id),
          eq(progressReviews.status, 'scheduled')
        )
      )
      .orderBy(progressReviews.scheduledDate)
      .limit(5);
      
    const participantReviews = await db.select()
      .from(progressReviewParticipants)
      .innerJoin(progressReviews, eq(progressReviewParticipants.reviewId, progressReviews.id))
      .where(
        and(
          eq(progressReviewParticipants.userId, user.id),
          eq(progressReviews.status, 'scheduled')
        )
      )
      .orderBy(progressReviews.scheduledDate)
...
router.get('/dashboard/stats', async (req: Request, res: Response) => {
  try {
    ...
    const counts = await Promise.all([
      db.select({ count: db.fn.count() }).from(progressReviews).where(eq(progressReviews.status, 'scheduled')),
      db.select({ count: db.fn.count() }).from(progressReviews).where(eq(progressReviews.status, 'in_progress')),
      db.select({ count: db.fn.count() }).from(progressReviews).where(eq(progressReviews.status, 'completed')),
      db.select({ count: db.fn.count() }).from(progressReviews).where(eq(progressReviews.status, 'cancelled'))
    ]);
    
    // Count action items by status
    const actionItemCounts = await Promise.all([
      db.select({ count: db.fn.count() }).from(progressReviewActionItems).where(eq(progressReviewActionItems.status, 'pending')),
      db.select({ count: db.fn.count() }).from(progressReviewActionItems).where(eq(progressReviewActionItems.status, 'in_progress')),
      db.select({ count: db.fn.count() }).from(progressReviewActionItems).where(eq(progressReviewActionItems.status, 'completed')),
      db.select({ count: db.fn.count() }).from(progressReviewActionItems).where(eq(progressReviewActionItems.status, 'cancelled'))
    ]);
