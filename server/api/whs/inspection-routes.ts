...
export function setupInspectionRoutes(router: express.Router) {
  ...
  router.get('/inspections', async (req, res) => {
    try {
      ...
      const inspections = await query.limit(limit).offset(offset);
  ...
  router.post('/inspections', hasPermission('whs.create_inspection'), async (req, res) => {
    try {
      ...
      // Create inspection
      const [newInspection] = await db.insert(whs_inspections).values(validatedData).returning();
  ...
  router.post(
    ...
    hasPermission('whs.update_inspection'),
    async (req, res) => {
      try {
