...
export function registerHostRoutes(app: Express) {
  ...
  app.get(
    '/api/host-agreements/:hostId',
    isAuthenticated,
    hasPermission('view:hosts'),
    async (req, res) => {
      try {
  ...
  app.get(
    '/api/host-agreements/:hostId/:agreementId',
    isAuthenticated,
    hasPermission('view:hosts'),
    async (req, res) => {
      try {
  ...
  app.delete('/api/host-agreements/:agreementId', async (req, res) => {
    try {
      ...
      await db.delete(hostEmployerAgreements).where(eq(hostEmployerAgreements.id, agreementId));
    ...
    async (req, res) => {
      try {
        ...
        const placementsData = await db
          ...
          .where(and(eq(placements.hostEmployerId, hostId), eq(placements.status, 'active')))
  ...
  app.get('/api/hosts/:hostId/stats', async (req, res) => {
    try {
      ...
      const completedApprenticesResult = await db
        .select({ count: sql<number>`count(distinct ${placements.apprenticeId})` })
        .from(placements)
        .where(and(eq(placements.hostEmployerId, hostId), eq(placements.status, 'completed')));

      // Get count of terminated apprentices
      const terminatedApprenticesResult = await db
        .select({ count: sql<number>`count(distinct ${placements.apprenticeId})` })
        .from(placements)
        .where(and(eq(placements.hostEmployerId, hostId), eq(placements.status, 'terminated')));
      ...
      const completionRate =
        totalPlaced > 0 ? Math.round((completed / (completed + terminated)) * 100) : 0;
      ...
      if (latestAgreement) {
        ...
        // Check facilities
        if (
          latestAgreement.supervisionCapacity &&
