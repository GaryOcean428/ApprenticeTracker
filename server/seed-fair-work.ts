...
export async function seedFairWorkData() {
  ...
  // Check if awards and enterprise agreements exist already
  const existingAwards = await db.select({ count: { value: awards.id } }).from(awards);
  const existingEAs = await db
    .select({ count: { value: enterpriseAgreements.id } })
    .from(enterpriseAgreements);

  const hasAwards = existingAwards[0]?.count.value > 0;
  const hasEnterpriseAgreements = existingEAs[0]?.count.value > 0;

  // If both already exist, skip everything
  if (hasAwards && hasEnterpriseAgreements) {
    console.log('Fair Work data already seeded. Skipping...');
    return;
  }

  // Insert awards only if they don't exist
  if (!hasAwards) {
    ...
    const sampleAwards = [
      {
        ...
        description:
          'Covers employers in on-site building, engineering and civil construction industries and their employees.',
      ...
      },
      {
        ...
        description:
          'Covers employers in the private sector who employ clerical and administrative employees.',
        effectiveDate: new Date('2020-11-01'),
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        ...
        description:
          'Covers employers in manufacturing and associated industries and their employees.',
        effectiveDate: new Date('2020-11-01'),
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        ...
        description:
          'Covers employers in electrical, electronic and communications contracting industry and their employees.',
        effectiveDate: new Date('2020-11-01'),
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date(),
      },
      {
        ...
        description:
          'Covers employers in the graphic arts, printing and publishing industries and their employees.',
        effectiveDate: new Date('2020-11-01'),
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date(),
    ...
    for (const award of sampleAwards) {
      ...
      if (inserted[0].id) {
        ...
        if (award.code === 'MA000020') {
        ...
        } else if (award.code === 'MA000002') {
          // Clerks Private Sector
          classifications = [
            { level: 'Level 1', name: 'Year 1', minHourlyRate: 21.54 },
            { level: 'Level 1', name: 'Year 2', minHourlyRate: 22.7 },
            ...
            { level: 'Level 4', name: 'Administrative Officer', minHourlyRate: 26.4 },
            { level: 'Level 5', name: 'Senior Administrative Officer', minHourlyRate: 27.5 },
          ];
        } else if (award.code === 'MA000010') {
          // Manufacturing
          classifications = [
            { level: 'C14', name: 'Engineering/Production Employee Level I', minHourlyRate: 21.33 },
            { level: 'C13', name: 'Engineering/Production Employee Level II', minHourlyRate: 22.1 },
            {
              level: 'C12',
              name: 'Engineering/Production Employee Level III',
              minHourlyRate: 23.33,
            },
            {
              level: 'C11',
              name: 'Engineering/Production Employee Level IV',
              minHourlyRate: 24.12,
            },
            ...
            {
              level: 'C8',
              name: 'Engineering Tradesperson Special Class Level I',
              minHourlyRate: 27.62,
            },
            {
              level: 'C7',
              name: 'Engineering Tradesperson Special Class Level II',
              minHourlyRate: 28.39,
            },
          ];
        } else {
          ...
          classifications = [
            ...
            { level: 'Level 2', name: 'Intermediate', minHourlyRate: 22.7 },
            { level: 'Level 3', name: 'Experienced', minHourlyRate: 24.4 },
  ...
  // Insert enterprise agreements only if they don't exist
  if (!hasEnterpriseAgreements) {
    console.log('Seeding enterprise agreements data...');
    ...
    const sampleEnterpriseAgreements = [
      {
        ...
        description:
          'Enterprise agreement for ABC Construction covering all construction trades and support staff.',
      ...
      },
      {
        ...
        description:
          'Enterprise agreement for XYZ Manufacturing covering production workers, maintenance, and warehouse staff.',
      ...
      },
      {
        ...
        description:
          'Enterprise agreement for City Metro Services covering administrative and customer service staff.',
      ...
      },
      {
        ...
        description:
          'Enterprise agreement for TechSolve covering developers, analysts, and IT support staff.',
    ...
    for (const agreement of sampleEnterpriseAgreements) {
      ...
      console.log(
        `Enterprise Agreement created: ${inserted[0].agreementName} (ID: ${inserted[0].id})`
      );
    }
  }
